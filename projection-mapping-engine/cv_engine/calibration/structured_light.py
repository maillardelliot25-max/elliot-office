...
# Canonical normalized positions the "Nudge Corners" overlay's draggable
# handles sit at before the operator moves anything (§3F). Order is fixed
# and mirrored exactly by `NudgeCornerLayout.canonicalPositions` in
# app/lib/core/models/nudge_points.dart — the Dart side sends back a plain
# ordered list of dragged positions, not a keyed structure, so this
# ordering is the only thing keeping the two sides in agreement.
NUDGE_CANONICAL_POSITIONS: dict[int, list[tuple[float, float]]] = {
...
def apply_manual_nudge(
    ...
    """Merges a manual "Nudge Corners" adjustment on top of a calibration
    mesh (auto-generated by Scan Room, or `identity_mesh()` if the operator
    hasn't scanned yet).

    `target_points` is the overlay's dragged handle positions — shape
    (point_count, 2), normalized [0,1] screen coordinates, in the fixed
    order given by `NUDGE_CANONICAL_POSITIONS[point_count]`. The per-handle
    displacement (dragged position minus its canonical rest position) is
    fit with the same thin-plate-spline approach `fit_deformation_mesh`
    uses for Scan Room, then added to `base_mesh` as a smooth correction —
    an untouched handle contributes zero displacement, and a dragged one's
    effect fades out smoothly toward the mesh's edges/other handles rather
    than only affecting a single grid cell.
    """
    from scipy.interpolate import RBFInterpolator

    if point_count not in NUDGE_CANONICAL_POSITIONS:
        raise ValueError(f"point_count must be 4 or 9, got {point_count}")

    canonical = np.array(NUDGE_CANONICAL_POSITIONS[point_count], dtype=np.float64)
    target_points = np.asarray(target_points, dtype=np.float64)
    if target_points.shape != canonical.shape:
        raise ValueError(
            f"expected target_points shape {canonical.shape}, got {target_points.shape}"
        )
    if base_mesh.shape != (mesh_resolution, mesh_resolution, 2):
        raise ValueError(
            f"expected base_mesh shape ({mesh_resolution}, {mesh_resolution}, 2), "
            f"got {base_mesh.shape}"
        )

    displacement = target_points - canonical

    coords = np.linspace(0.0, 1.0, mesh_resolution)
    grid_x, grid_y = np.meshgrid(coords, coords)
    query = np.stack([grid_x.ravel(), grid_y.ravel()], axis=-1)

    interpolator = RBFInterpolator(canonical, displacement, kernel="thin_plate_spline")
    displacement_field = interpolator(query).reshape(mesh_resolution, mesh_resolution, 2)

    nudged = base_mesh + displacement_field.astype(np.float32)
    return np.clip(nudged, 0.0, 1.0).astype(np.float32)
